...
docker build -f Dockerfile_train_models -t allora-train-model:1.0.1 .
...
docker run --gpus all -v $(pwd)/models:/app/models -v $(pwd)/source-data:/app/data -e DATABASE_PATH=/app/data/prices.db --name allora-train-model allora-train-model:1.0.1 bash -c "python /app/train_models.py"
