...
# Cài đặt thêm scikit-learn nếu chưa có

docker run --gpus all -v $(pwd)/models:/app/models -v $(pwd)/source-data:/app/data -e DATABASE_PATH=/app/data/prices.db --name allora-train-model allora-train-model:1.0.0 bash -c "pip install scikit-learn && python /app/train_models.py"
