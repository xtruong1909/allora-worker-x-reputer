...
import functools
import json  # For manually creating JSON responses
from app_config import DATABASE_PATH

# Kiểm tra và thông báo GPU đã được nhận diện
...
# Tạo ThreadPoolExecutor cho các tác vụ đồng thời
executor = ThreadPoolExecutor()

# Tạo kết nối SQLite (check_same_thread=False cho đa luồng)
...
def load_model_and_scaler(token_name, prediction_horizon):
    ...
    if not os.path.exists(model_path) or not os.path.exists(scaler_path):
        print(f"Model or scaler not found for {token_name} with horizon {prediction_horizon} minutes.")
        return None, None
...
def get_inference(token):
    ...
    def make_prediction():
        return cached_prediction(token_name, PREDICTION_STEPS)

    try:
        mean_forecast = executor.submit(make_prediction).result()
